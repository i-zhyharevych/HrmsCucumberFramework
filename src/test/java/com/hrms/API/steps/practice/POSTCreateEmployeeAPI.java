...
public class POSTCreateEmployeeAPI {
	...
	public void user_calls_createEmployee_API_to_verify_that_an_employee_was_created() {
		...
		request = given().header("Content_type", "application/json").header("Authorization",
				SyntaxAPIAuthentificationSteps.Token);
		request.body(CommonMethods.readJson(APIConstants.CREATE_EMPLOYEE_JSON)); //path to the createEmployee.json file
	...
	public void user_validates_employee_was_created() {
		...
		response.then().body("Employee[0].emp_middle_name", equalTo("J"));
		
		response.then().body("Employee[0].emp_lastname", equalTo("Z"));
